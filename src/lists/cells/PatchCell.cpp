...
void PatchCell::init(const size_t index, Patch* patch) {
    ...
    BetterTextArea<false>* description = BetterTextArea<false>::create("chatFont.fnt", patch->m_description, 1, this->m_fWidth - 80);
    ...
    GameToolbox::createToggleButton(menu_selector(PatchCell::onEnable), patch->m_enabled, menu, this, this->m_pLayer, 1, 1, 160, "", false, 0, nullptr, "", {
        this->m_fWidth - 30,
        this->m_fHeight - 48
    }, { 0, 0 });
    ...
    name->setPosition({ 15, this->m_fHeight - 10 });
    interface->setSizeMult(1.5f);
    interfaceMenu->setScale(0.7f);
    interfaceMenu->setAnchorPoint({ 0, 0 });
    interfaceMenu->setPosition({ 27 + name->getContentSize().width * 0.7f, this->m_fHeight - 21 });
    ...
    this->m_pLayer->addChild(interfaceMenu);
